...
class AdminOrderController extends Controller
{
    ...
    function list(Request $request)
    {
        ...
        if ($status == 'trash') {
            ...
            $orders = Order::onlyTrashed()->orderBy('created_at', 'desc')->simplepaginate(10);
        } else if ($status == 0) {
            $orders = Order::where('status', '0')->orderBy('created_at', 'desc')->simplepaginate(10);
        } else if ($status == 1) {
            $orders = Order::where('status', '1')->orderBy('created_at', 'desc')->simplepaginate(10);
        } else if ($status == 2) {
            $orders = Order::where('status', '2')->orderBy('created_at', 'desc')->simplepaginate(10);
        } else if ($status == 3) {
            $orders = Order::where('status', '3')->orderBy('created_at', 'desc')->simplepaginate(10);
        } else {
            $keyword = '';
            if ($request->input('keyword')) {
                $keyword = $request->input('keyword');
            }
            $orders = Order::where('code', 'LIKE', "%{$keyword}%")->orderBy('created_at', 'desc')->simplepaginate(10);
        }
